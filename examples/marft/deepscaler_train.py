...
Usage::
    ...
    # Single agent
    python examples/marft/deepscaler_train.py \\
        --config examples/marft/deepscaler_1agent.yaml \\
        scheduler.type=local

    # Multi-agent (2 / 3 / 4 agents)
    python examples/marft/deepscaler_train.py \\
        --config examples/marft/deepscaler_2agent.yaml \\
        scheduler.type=local
"""
...
def _build_multi_agent_workflow_kwargs(config: PPOConfig) -> dict:
    ...
    Mirrors the logic in ``examples/multi_agent_math/gsm8k_ma_lora.py``:
