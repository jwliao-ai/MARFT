"""DeepCoder 2-agent RL with per-agent LoRA adapters.

Two agents (planner → solver) collaborate on each coding problem via a
fixed sequential graph.  Each agent is backed by its own named LoRA
adapter on a shared base model.

This is a simplified pipeline compared to the 3-agent variant — no
debugger.  The hypothesis is that fewer agents mean clearer reward
attribution and less context wasted, while the planner still adds
value by structuring the approach before the solver writes code.

Uses PPO with a critic (the MARFT backbone), matching the 3-agent
config.  The team reward is the code execution pass rate on the
problem's test cases.  The solver's final ```python``` block is
extracted and evaluated.

Usage::

    python examples/deepcoder/deepcoder_ma_lora_2agent.py \
        --config examples/deepcoder/deepcoder_ma_lora_2agent.yaml \
