...
def concat_padded_tensors(
    ...
    for key in tensor_dicts[0].keys():
        ...
        if is_multi_modal_key(key):
            continue
        # Skip non-tensor metadata (e.g. agent_names from multi-agent workflows).
        # Use hasattr(_, 'shape') instead of torch.is_tensor() because RTensor
        # (remote tensor dataclass) is not a torch.Tensor subclass.
        if not hasattr(tensor_dicts[0][key], "shape"):
            result[key] = tensor_dicts[0][key]
            continue
