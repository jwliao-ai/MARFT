...
class RemoteInfBackendProtocol(Protocol):
    ...
    def build_disk_weight_update_requests(
        ...
        loaded_adapters : set[str]
            Names of LoRA adapters already loaded on the server.
            An adapter must be unloaded before re-loading it with
            updated weights; adapters not yet loaded are loaded directly.
    ...
    def update_weights_from_disk(self, meta: WeightUpdateMeta) -> Future[None]:
        ...
        def callback(fut):
            ...
            if meta.use_lora:
                ...
                if meta.lora_names:
                    self.loaded_adapters.update(meta.lora_names)
                else:
                    adapter_name = meta.lora_name or "lora_1"
...
def _update_weights_from_disk(
    ...
    async def _fn():
        ...
        weight_reqs = backend.build_disk_weight_update_requests(meta, loaded_adapters)
