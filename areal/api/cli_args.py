...
class SchedulingSpec:
    ...
    image: str = field(
        default="/storage/cached/images/areal-latest.sif",
        metadata={
...
class TrainEngineConfig:
    ...
    multi_lora_names: list[str] = field(
        default_factory=list,
        metadata={
            "help": "Per-agent LoRA adapter names for multi-adapter training. "
            "When non-empty, each adapter is trained independently with its "
            "own subset of tokens (selected via agent_ids). "
            "Populated automatically from multi_agent.get_lora_names() "
            "when shared_lora=False."
        },
    )
...
class PPOActorConfig(TrainEngineConfig):
    ...
    # Multi-agent reward sharing
    reward_sharing: bool = field(
        default=True,
        metadata={
            "help": "When True, a single team reward at the last response token "
            "drives advantage computation (standard PPO). When False, per-agent "
            "rewards from token_rewards are used so each agent's advantage is "
            "derived from its own reward signal; subsequent agent rewards still "
            "propagate backward through GAE and the critic."
        },
    )
...
class PPOCriticConfig(TrainEngineConfig):
    ...
    num_critic_heads: int = field(
        default=1,
        metadata={
            "help": "Number of per-agent value heads when using multi_head "
            "independent critic mode. Set automatically from the number of "
            "agents; manual override is rarely needed."
        },
    )
    multi_lora_names: list[str] = field(
        default_factory=list,
        metadata={
            "help": "Per-agent LoRA adapter names for multi-adapter critic "
            "training. Populated automatically from multi_agent.get_lora_names() "
            "when independent_critic='lora'."
        },
    )
...
def _create_dummy_lora_for_sglang(
    model_path: str,
    lora_rank: int,
    target_modules: list[str],
    lora_alpha: int | None = None,
) -> str | None:
    """Create a minimal LoRA adapter so SGLang allocates its LoRA memory pool.

    SGLang needs at least one adapter loaded via ``--lora-paths`` at startup
    to initialise the LoRA memory pool.  When no real adapter exists yet
    (e.g. before the first training step), this helper writes a dummy adapter
    that satisfies the requirement.

    The adapter is created directly (no PEFT dependency at runtime) by reading
    the model config for architecture info and writing zero-initialised LoRA
    weight tensors in PEFT-compatible safetensors format.

    Returns the path to the dummy adapter directory, or *None* on failure.
    """
    import tempfile
    ...
    cfg_path = os.path.join(dummy_dir, "adapter_config.json")
    weights_path = os.path.join(dummy_dir, "adapter_model.safetensors")
    if os.path.isfile(cfg_path) and os.path.isfile(weights_path):
        return dummy_dir
    ...
    try:
        ...
        # Resolve target modules: "all" → standard linear projection list.
        if target_modules == ["all"] or target_modules == ["all-linear"]:
            resolved = [
                "q_proj",
                "k_proj",
                "v_proj",
        ...
        # (in_features, out_features) for each module.
        module_dims = {
        ...
        # Attention modules live under self_attn, MLP under mlp.
        ...
        for layer_idx in range(n_layers):
            for mod in resolved:
                in_feat, out_feat = module_dims[mod]
                if mod in attn_modules:
                    prefix = (
                        f"base_model.model.model.layers.{layer_idx}.self_attn.{mod}"
                    )
                elif mod in mlp_modules:
                    prefix = f"base_model.model.model.layers.{layer_idx}.mlp.{mod}"
                else:
                    continue
                # lora_A: [rank, in_features], lora_B: [out_features, rank]
                tensors[f"{prefix}.lora_A.weight"] = torch.zeros(
                    lora_rank, in_feat, dtype=torch.bfloat16
                )
                tensors[f"{prefix}.lora_B.weight"] = torch.zeros(
                    out_feat, lora_rank, dtype=torch.bfloat16
                )
...
class SGLangConfig:
    ...
    def build_args(
        ...
        # When LoRA is enabled but no lora_paths are specified,
        # create a dummy adapter so SGLang allocates its LoRA memory
        # pool at startup (required for dynamic /load_lora_adapter).
        if (
            sglang_config.enable_lora
            and not sglang_config.lora_paths
            and sglang_config.max_lora_rank
            and sglang_config.model_path
        ):
            dummy_path = _create_dummy_lora_for_sglang(
                model_path=sglang_config.model_path,
                lora_rank=sglang_config.max_lora_rank,
                target_modules=sglang_config.lora_target_modules or ["all"],
            )
            if dummy_path:
                args["lora_paths"] = [f"__dummy__={dummy_path}"]
...
class MultiAgentConfig:
    """Configuration for multi-agent cooperative workflows."""

    graph_type: str = field(
        default="sequential",
        metadata={
            "help": "Graph topology type.",
            "choices": ["sequential", "custom"],
        },
    )
    role_names: list[str] | None = field(
        default=None,
        metadata={
            "help": "Ordered role names for sequential graphs "
            "(e.g., ['planner', 'solver', 'verifier'])."
        },
    )
    role_configs: dict | None = field(
        default=None,
        metadata={
            "help": "Per-role configuration dicts keyed by role name.  "
            "Each value may contain: system_prompt, description, "
            "max_new_tokens, temperature, top_p, lora_name."
        },
    )
    graph_config: dict | None = field(
        default=None,
        metadata={
            "help": "Full graph config (nodes + edges) for custom topologies. "
            "Ignored when graph_type='sequential'."
        },
    )
    transition_messages: list[str] | None = field(
        default=None,
        metadata={
            "help": "Per-step user messages inserted between agents "
            "in a sequential graph.  Same length as role_names."
        },
    )
    credit_strategy: str = field(
        ...
        metadata={
            ...
            "choices": ["equal", "step_discount", "per_step"],
        },
    )
    credit_discount: float = field(
        default=1.0,
        metadata={"help": "Discount factor for step_discount credit strategy."},
    )
    step_reward_fn: str | None = field(
        default=None,
        metadata={
            "help": "Import path for per-step reward function "
            "(required when credit_strategy='per_step')."
        },
    )
    shared_policy: bool = field(
        default=True,
        metadata={"help": "Whether all agents share a single policy model."},
    )
    enable_thinking: bool = field(
        default=False,
        metadata={"help": "Pass enable_thinking=True to apply_chat_template."},
    )
    dump_transcripts: bool = field(
        default=False,
        metadata={"help": "Persist multi-agent conversation transcripts to dump_dir."},
    )
    orchestration_mode: str = field(
        default="static",
        metadata={
            "help": "Orchestration mode: 'static' (fixed DAG) or 'dynamic' "
            "(LLM orchestrator decides agent order).",
            "choices": ["static", "dynamic"],
        },
    )
    orchestrator_prompt: str | None = field(
        default=None,
        metadata={
            "help": "Custom system prompt for the dynamic orchestrator. "
            "If None, an auto-generated prompt listing available agents is used."
        },
    )
    orchestrator_max_new_tokens: int | None = field(
        default=None,
        metadata={
            "help": "Max new tokens for orchestrator generation steps. "
            "Overrides gconfig.max_new_tokens for the orchestrator only."
        },
    )
    max_steps: int = field(
        default=10,
        metadata={
            "help": "Maximum orchestrator→agent interaction steps "
            "in dynamic mode before forced termination."
        },
    )
    ...
    use_multi_lora: bool = field(
        default=False,
        metadata={
            "help": "Enable per-agent LoRA adapters. When True, each role gets "
            "its own named adapter on the inference server. Requires actor.use_lora=True "
            "and sglang.max_loaded_loras >= number of agents."
        },
    )
    lora_name_prefix: str = field(
        default="agent",
        metadata={
            "help": "Prefix for auto-generated adapter names. "
            "Agent 'solver' becomes '{prefix}_solver'. "
            "Ignored when role_configs specify explicit lora_name."
        },
    )
    shared_lora: bool = field(
        default=True,
        metadata={
            "help": "When True (with use_multi_lora), all agents share a single "
            "LoRA adapter during training but inference uses per-agent adapter "
            "names (loaded from the same weights). Set to False for true "
            "per-agent adapter specialization where each agent trains its own "
            "LoRA weights independently."
        },
    )
    reward_sharing: bool = field(
        default=True,
        metadata={
            "help": "When True, a single team reward is placed at the last "
            "response token and the critic + GAE handle all credit assignment. "
            "When False, each agent receives its own reward (via token_rewards "
            "from CreditAssignment) and advantages are computed per-agent; "
            "subsequent agent rewards still propagate backward through GAE."
        },
    )
    independent_critic: str | None = field(
        default=None,
        metadata={
            "help": "Per-agent independent critic mode for Independent Learning. "
            "'lora' = per-agent LoRA adapters on a shared critic base model. "
            "'multi_head' = shared backbone with N separate score heads "
            "(nn.Linear per agent). None = single shared critic (default CTDE).",
            "choices": [None, "lora", "multi_head"],
        },
    )

    def get_lora_names(self) -> list[str]:
        """Return per-agent LoRA adapter names when use_multi_lora is enabled."""
        if not self.use_multi_lora:
            return []
        prefix = self.lora_name_prefix or "agent"
        names = self.role_names or list((self.role_configs or {}).keys())
...
class PPOConfig(BaseExperimentConfig):
    ...
    multi_agent: MultiAgentConfig = field(
        ...
        metadata={"help": "Multi-agent cooperative workflow configuration."},
    )
