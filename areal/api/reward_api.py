    ...
    async def __call__(self, *args, **kwargs) -> float:
        ...
        for attempt in range(self.max_retries + 1):
            ...
            try:
                ...
                # math_verify.errors.TimeoutException inherits from
                # BaseException (not Exception) and escapes the handler
                # above.  Catch it here as a safety net so a single bad
                # sample never crashes the whole rollout pipeline.
                logger.warning(
                    f"BaseException in reward computation (returning 0): {e}"
                )
                return 0.0
