...
class FSDPEngine(TrainEngine):
    ...
    def _apply_peft_wrapper(self):
        ...
        task_type = TaskType.TOKEN_CLS if config.is_critic else TaskType.CAUSAL_LM
    ...
    def _save_model_to_hf(
        ...
        if dist.get_rank() == 0:
            if self.config.use_lora:
                # Wipe directory to remove stale files from previous
                # iterations (tokenizer files, config.json, etc.).
                # SGLang's LoRAConfig reads added_tokens.json if present
                # and reports non-zero lora_added_tokens_size, which fails
                # the memory pool compatibility check in can_support().
                ...
                self.logger.info(
                    f"LoRA save: {len(lora_keys)} adapter keys out of "
                    f"{len(state_dict)} total keys. "
                    f"Sample keys: {lora_keys[:3]}"
                )
                self.model.save_pretrained(path, state_dict=state_dict)
                saved_files = os.listdir(path)
                self.logger.info(f"LoRA save to {path} produced files: {saved_files}")
            else:
                os.makedirs(path, exist_ok=True)
                self.model.save_pretrained(path, state_dict=state_dict)
...
class MultiAdapterFSDPPPOActor(FSDPPPOActor):
    """PPO Actor with per-agent LoRA adapters for multi-agent training.

    Each agent gets its own named LoRA adapter.  During ``train_batch``,
    the loss mask is filtered per-adapter so that each adapter only
    receives gradients from tokens produced by its corresponding agent.
    All adapters share a single optimizer and learning-rate schedule.
    """

    def _apply_peft_wrapper(self):
        """Create the default LoRA adapter, then add per-agent adapters.

        By adding adapters here (before ``parallelize_model`` and
        ``_create_optimizer`` in the parent ``initialize``), all adapter
        parameters are visible to FSDP sharding and included in the
        optimizer's parameter groups.
        """
        super()._apply_peft_wrapper()
        adapter_names = self.config.multi_lora_names
        if adapter_names:
            from areal.engine.multi_adapter_mixin import MultiAdapterMixin

            self._multi_adapter_cls = MultiAdapterMixin
            MultiAdapterMixin.setup_multi_adapter(self, adapter_names)
        else:
            self._multi_adapter_cls = None
    ...
    def forward_batch(
        ...
        """Compute logprobs per-adapter and merge based on ``agent_ids``.

        When ``compute_logp`` triggers a forward pass (e.g. proximal
        log-prob recomputation), each adapter must produce logprobs only
        for its own tokens.  This override runs one forward pass per
        adapter and stitches the results together using ``agent_ids``.
        """
        if self._multi_adapter_cls is None or "agent_ids" not in input_:
            return super().forward_batch(input_, output_seqlens, aggregate_fn)

        agent_ids = input_["agent_ids"]
        combined: torch.Tensor | None = None

        for adapter_idx, adapter_name in enumerate(self._adapter_names):
            self._multi_adapter_cls.set_active_adapter(self, adapter_name)
            logprobs = super().forward_batch(dict(input_), output_seqlens, aggregate_fn)
            if combined is None:
                combined = torch.zeros_like(logprobs)
            mask = agent_ids == adapter_idx
            if mask.shape != combined.shape:
                mask = mask.view(combined.shape)
            combined[mask] = logprobs[mask]
    ...
    def train_batch(
        ...
        # loss_mask was rolled by -1 in compute_advantages; roll agent_ids
        # the same way so the per-adapter mask aligns with the shifted
        # loss positions (position t predicts token t+1).
        agent_ids = torch.roll(input_["agent_ids"], shifts=-1, dims=-1)
        original_loss_mask = input_["loss_mask"]

        for adapter_idx, adapter_name in enumerate(self._adapter_names):
            self._multi_adapter_cls.set_active_adapter(self, adapter_name)

            adapter_mask = self._multi_adapter_cls.get_adapter_loss_mask(
                original_loss_mask,
                agent_ids,
                adapter_idx,
            ).float()

            input_["loss_mask"] = adapter_mask

            mb_list = self._prepare_mb_list(input_).to(self.device)
            # total_loss_weight is all-reduced across DP ranks.  When it
            # is zero every rank agrees that no tokens belong to this
            # adapter, so we can safely skip.  Never skip based on a
            # local check alone — that causes NCCL rank divergence.
            total_loss_weight = compute_total_loss_weight(
                mb_list, loss_weight_fn, self.dp_group
            )
        ...
        input_["loss_mask"] = original_loss_mask
        return self.optimizer_step()

    def update_weights(self, meta: WeightUpdateMeta):
        if self._multi_adapter_cls is None:
            return super().update_weights(meta)
        ...
        if meta.type == "xccl":
            ...
            with tms_context:
                self._multi_adapter_cls.update_multi_adapter_weights(self, meta)
        elif meta.type == "disk":
            self._update_weights_from_disk(meta)
        else:
            raise ValueError(f"Unknown weight update type {meta.type}")
    ...
    def _load_model_from_hf(self, path: str):
        """Load per-agent LoRA adapters from HuggingFace format.

        Expects the directory structure produced by ``_save_model_to_hf``:
        ``path/{adapter_name}/adapter_model.safetensors`` for each adapter.
        Keys stored as ``.default.`` are renamed back to ``.{adapter_name}.``
        before feeding to ``fsdp2_load_full_state_dict``.
        """
        if self._multi_adapter_cls is None:
            return super()._load_model_from_hf(path)
        ...
        if dist.get_rank() == 0:
            ...
            for adapter_name in self._adapter_names:
                adapter_path = os.path.join(
                    path, adapter_name, "adapter_model.safetensors"
                )
                if not os.path.exists(adapter_path):
                    ...
                    continue
                adapter_state = safetensors_load(adapter_path)
                for key, value in adapter_state.items():
                    real_key = key.replace(".default.", f".{adapter_name}.")
                    full_state[real_key] = value
                self.logger.info(
                    f"Per-adapter load: '{adapter_name}' <- {adapter_path} "
                    f"({len(adapter_state)} keys)"
                )
    ...
    def _save_model_to_hf(
        ...
        if self._multi_adapter_cls is None:
            return super()._save_model_to_hf(path, tokenizer, processor)
        ...
        if dist.get_rank() == 0:
            ...
            peft_config = self.model.peft_config[adapter_names[0]]
            config_dict = peft_config.to_dict()
            # PEFT stores target_modules as a set which is not JSON
            # serializable.  Convert sets to sorted lists.
            for key, val in config_dict.items():
                if isinstance(val, set):
                    config_dict[key] = sorted(val)
            ...
            for adapter_name in adapter_names:
                ...
                for key, value in state_dict.items():
                    if f".{adapter_name}." in key and "lora_" in key:
                        new_key = key.replace(f".{adapter_name}.", ".default.")
                ...
                with open(
                    os.path.join(adapter_path, "adapter_config.json"), "w"
                ) as cfg_f:
                    json.dump(config_dict, cfg_f, indent=2)

                self.logger.info(
                    f"Per-adapter save: '{adapter_name}' -> {adapter_path} "
                    f"({len(adapter_state)} keys)"
                )
...
class MultiHeadFSDPPPOCritic(FSDPPPOCritic):
    """PPO Critic with per-agent value heads for Independent Learning.

    Instead of a single ``nn.Linear(hidden_size, 1)`` score layer, the model
    is created with ``num_labels=N`` (one per agent), yielding
    ``nn.Linear(hidden_size, N)``.  During ``compute_values``, each token's
    value is gathered from the head corresponding to its ``agent_ids`` entry.
    During training, gradients flow only through each token's selected head.
    """

    def _create_llm_actor_or_critic(self):
        dtype = getattr(torch, self.config.dtype)

        num_heads = self.config.num_critic_heads
        if num_heads < 2:
            raise ValueError(
                f"MultiHeadFSDPPPOCritic requires num_critic_heads >= 2, "
                f"got {num_heads}"
            )

        model_class = AutoModelForTokenClassification
        model_kwargs = {"num_labels": num_heads}
        common_kwargs = {
            "dtype": dtype,
            "attn_implementation": self.config.attn_impl,
        }
        ...
        # Disable dropout throughout the model
        disable_dropout_in_model(model)
        ...
        self.logger.info(
            f"Multi-head critic created with {num_heads} value heads "
            f"(score layer: Linear({self.model_config.hidden_size}, {num_heads}))"
        )
        self.model = model
    ...
    def _compute_forward_result(
        ...
        agent_ids = ctx.mb_input.get("agent_ids")
        if agent_ids is None:
            return super()._compute_forward_result(logits, ctx)

        # logits: [seq_len, num_heads] (after squeeze(0) of batch dim)
        # Gather the value for each token's assigned agent head
        if logits.dim() == 1:
            # Single-head fallback (should not happen in multi-head mode)
            values = logits
        else:
            agent_ids_flat = agent_ids.view(-1)
            if agent_ids_flat.shape[0] != logits.shape[0]:
                agent_ids_flat = agent_ids_flat[: logits.shape[0]]
            values = logits.gather(
                -1, agent_ids_flat.unsqueeze(-1).to(logits.device)
            ).squeeze(-1)

        values = self._compute_values(values, ctx.ulysses_pad_size)
        if ctx.pad_length > 0:
            values = values[: -ctx.pad_length]
        return values
    ...
    def _compute_logprobs_and_loss(
        ...
        # logits: [seq_len, num_heads] — gather to get per-token values
        agent_ids_flat = agent_ids.view(-1)
        if agent_ids_flat.shape[0] != logits.shape[0]:
            agent_ids_flat = agent_ids_flat[: logits.shape[0]]
        values = logits.gather(
...
class MultiAdapterFSDPPPOCritic(FSDPPPOCritic):
    """PPO Critic with per-agent LoRA adapters for Independent Learning.

    Each agent gets its own LoRA adapter on the shared critic backbone.
    During ``compute_values``, each adapter is activated in turn and produces
    values only for its agent's tokens.  During training, loss is masked
    per-adapter so gradients flow through only the relevant adapter.
    All adapters share a single optimizer and learning-rate schedule.
    """
    ...
    def _apply_peft_wrapper(self):
        ...
        if adapter_names:
            ...
            self._multi_adapter_cls = MultiAdapterMixin
            MultiAdapterMixin.setup_multi_adapter_critic(self, adapter_names)
        else:
            self._multi_adapter_cls = None
    ...
    def compute_values(self, *args, **kwargs) -> torch.Tensor:
        if self._multi_adapter_cls is None:
            return super().compute_values(*args, **kwargs)
        return self.critic.compute_values(*args, **kwargs)
    ...
    def forward_batch(
        ...
        for adapter_idx, adapter_name in enumerate(self._adapter_names):
            self._multi_adapter_cls.set_active_adapter(self, adapter_name)
            values = super().forward_batch(dict(input_), output_seqlens, aggregate_fn)
            if combined is None:
                combined = torch.zeros_like(values)
            mask = agent_ids == adapter_idx
            if mask.shape != combined.shape:
                mask = mask.view(combined.shape)
            combined[mask] = values[mask]
    ...
    def train_batch(
        ...
        # Unlike the actor (which rolls agent_ids to align with the
        # next-token prediction target), the critic must use un-rolled
        # agent_ids to stay consistent with compute_values.  The value
        # V(s_t) at position t was estimated by agent_t's adapter during
        # compute_values; the training target (returns = advantages +
        # values) was computed from those same values.  Using a different
        # adapter here would create a target mismatch.
        agent_ids = input_["agent_ids"]
        original_loss_mask = input_["loss_mask"]
    ...
    def _load_model_from_hf(self, path: str):
        """Load per-agent critic LoRA adapters from HuggingFace format.

        Mirrors ``MultiAdapterFSDPPPOActor._load_model_from_hf``.
        """
        if self._multi_adapter_cls is None:
            return super()._load_model_from_hf(path)
        ...
        if dist.get_rank() == 0:
            ...
            for adapter_name in self._adapter_names:
                ...
                for key, value in adapter_state.items():
                    ...
                    full_state[real_key] = value
                self.logger.info(
                    f"Per-adapter critic load: '{adapter_name}' <- {adapter_path} "
                    f"({len(adapter_state)} keys)"
                )
    ...
    def _save_model_to_hf(
        ...
        if dist.get_rank() == 0:
            ...
            for adapter_name in adapter_names:
                ...
                self.logger.info(
                    f"Per-adapter critic save: '{adapter_name}' -> {adapter_path} "
                    f"({len(adapter_state)} keys)"
                )
