...
class SGLangBackend:
    ...
    def build_generation_request(
        ...
        # Add LoRA if initialized — use per-request adapter name if provided,
        # otherwise fall back to the default single-adapter name.
        if with_lora:
            lora_name = getattr(gconfig, "lora_name", "") or "lora_1"
    ...
    def build_disk_weight_update_requests(
        self, meta: WeightUpdateMeta, loaded_adapters: set[str]
    ) -> WeightUpdateRequests:
        """Build SGLang disk weight update requests."""
        if meta.use_lora:
            # Determine adapter names: use per-agent names if available,
            # otherwise fall back to the single lora_name.
            lora_names = (
                meta.lora_names if meta.lora_names else [meta.lora_name or "lora_1"]
            )
            requests = []
            # Unload previously loaded adapters first
            ...
            # Load all adapters.  When per_adapter_subdirs is set each
            # adapter lives under its own subdirectory of meta.path;
            # otherwise they all share the same path (shared_lora).
            for name in lora_names:
                lora_path = (
                    os.path.join(str(meta.path), name)
                    if meta.per_adapter_subdirs
                    else str(meta.path)
                )
                requests.append(
                    HttpRequest(
