...
class PPOActor:
    ...
    def compute_advantages(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        if self.config.reward_sharing:
            # Standard PPO: single team reward at the penultimate token.
            ...
            # Per-agent rewards: use token_rewards from CreditAssignment.
            # Each agent's reward is placed at its last generated token.
            token_rewards = data.get("token_rewards")
            if token_rewards is None:
                raise ValueError(
                    "reward_sharing=False requires token_rewards in rollout data. "
                    "Ensure the workflow produces per-agent token_rewards via "
                    "CreditAssignment."
                )
            token_rewards = token_rewards.to(rewards.device).float()
            nonzero_mask = token_rewards != 0

            # Scale and clip each per-agent reward entry.
            scaled = (token_rewards + self.reward_bias) * self.reward_scaling
            scaled = torch.clip(scaled, min=-self.reward_clip, max=self.reward_clip)

            # Per-entry batch normalization: normalize all non-zero entries
            # across the batch so that agents with different reward magnitudes
            # are each brought to a comparable scale.
            if self.reward_norm and nonzero_mask.any():
                flat_entries = scaled[nonzero_mask]
                flat_normed = self.reward_norm(flat_entries.unsqueeze(0)).squeeze(0)
                scaled = scaled.clone()
                scaled[nonzero_mask] = flat_normed

            per_agent_rewards = torch.where(
                nonzero_mask, scaled, torch.zeros_like(scaled)
            )

            if self.mask_no_eos_with_zero:
                eos_mask = (~seq_no_eos_mask).unsqueeze(-1).expand_as(per_agent_rewards)
                per_agent_rewards = per_agent_rewards * eos_mask.float()
