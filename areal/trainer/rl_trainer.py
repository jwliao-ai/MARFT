...
class PPOTrainer:
    def __init__(
        ...
        # Populate per-agent LoRA adapter names when shared_lora=False.
        if config.multi_agent.use_multi_lora and not config.multi_agent.shared_lora:
            config.actor.multi_lora_names = config.multi_agent.get_lora_names()

        # Populate independent critic config from multi_agent settings.
        ic_mode = config.multi_agent.independent_critic
        if ic_mode is not None and config.critic is not None:
            num_agents = len(config.multi_agent.role_names or [])
            if num_agents < 2:
                raise ValueError(
                    f"independent_critic='{ic_mode}' requires at least 2 agents "
                    f"(multi_agent.role_names), got {num_agents}"
                )
            if ic_mode == "multi_head":
                config.critic.num_critic_heads = num_agents
            elif ic_mode == "lora":
                config.critic.multi_lora_names = config.multi_agent.get_lora_names()
            else:
                raise ValueError(
                    f"Unknown independent_critic mode: '{ic_mode}'. "
                    f"Expected 'lora', 'multi_head', or None."
                )
        ...
        if self.config.actor.weight_update_mode == "disk":
            ...
            if config.actor.use_lora:
                ...
                # Only register per-agent names when each agent has its own
                # independent adapter (shared_lora=False).  With shared_lora=True
                # all agents point at the same single checkpoint; loading the
                # same path under multiple names corrupts SGLang's LoRA registry
                # and causes cudaErrorIllegalAddress on the next prefill.
                if not config.multi_agent.shared_lora:
                    lora_names = config.multi_agent.get_lora_names()
                    if lora_names:
                        disk_kwargs["lora_names"] = lora_names
            ...
            if self.allocation_mode.train_backend == "megatron":
                ...
                if config.actor.use_lora:
                    ...
                    # Same shared_lora guard as the disk path above.
                    if not config.multi_agent.shared_lora:
                        lora_names = config.multi_agent.get_lora_names()
                        if lora_names:
                            xccl_kwargs["lora_names"] = lora_names
        ...
        # When per-agent LoRA adapters are trained independently each adapter
        # is saved to its own subdirectory so SGLang can load them separately.
        if config.actor.multi_lora_names:
            self.weight_update_meta.per_adapter_subdirs = True
        ...
        recover_engines: dict = {"default": self.actor}
        if self.critic is not None:
    ...
    def _create_actor(
        ...
        if self.allocation_mode.train_backend == "fsdp":
            if actor_config.multi_lora_names:
                from areal.engine.fsdp_engine import MultiAdapterFSDPPPOActor

                actor_cls = MultiAdapterFSDPPPOActor
            else:
    ...
    def _create_critic(
        ...
        if self.allocation_mode.train_backend == "fsdp":
            critic_cls = self._resolve_fsdp_critic_cls(critic_config)
        elif self.allocation_mode.train_backend == "megatron":
    ...
    @staticmethod
    def _resolve_fsdp_critic_cls(critic_config: PPOCriticConfig):
        if critic_config.num_critic_heads > 1:
            from areal.engine.fsdp_engine import MultiHeadFSDPPPOCritic

            return MultiHeadFSDPPPOCritic
        if critic_config.multi_lora_names:
            from areal.engine.fsdp_engine import MultiAdapterFSDPPPOCritic

            return MultiAdapterFSDPPPOCritic
        from areal.engine.fsdp_engine import FSDPPPOCritic

        return FSDPPPOCritic
    ...
    def _save_hf(self, epoch: int, epoch_step: int, global_step: int):
        # Check save frequency once to avoid double-counting (actor + critic
        # both call saver.save which would increment the freq counter twice).
        if not self.saver.should_save(epoch_step):
            return
